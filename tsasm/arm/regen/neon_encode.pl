...
# VREV32 -- A8.6.354. Reverses 16-bit halfwords within each 32-bit word
# (or 8-bit bytes within 32-bit words).
# 1111 0011 1D11 ss 00 Vd 0000 op Q M 0 Vm
# For .16: op=1; for .8: op=0. ss=size of element (2 bits).
sub enc_vrev32 {
    my ($mn, $args) = @_;
    my @a = split /\s*,\s*/, $args;
...
# vld1 / vst1 with various size suffixes.
$PREFIX_ENCODERS{"vld1."} = \&enc_vld1;
$PREFIX_ENCODERS{"vst1."} = \&enc_vst1;
